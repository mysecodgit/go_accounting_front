  ...
  const calculateSplits = (itemsList) => {
    ...
    itemsList.forEach((invoiceItem) => {
      ...
      if (item.type === "discount") {
        ...
        // Add service items to service total (for income credit)
        serviceTotalAmount += itemTotal;
        // Group service income by account
        if (item.income_account?.id) {
    ...
    const arDebitAmount = serviceTotalAmount - discountTotal - paymentTotal;

    // Debit: Accounts Receivable (remaining amount after discount and payment)
    if (arDebitAmount > 0) {
      splits.push({
    ...
    // Credit: Service Income Accounts
